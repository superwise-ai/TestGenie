...
export default function EditTestCasePage() {
      ...
      try {
        ...
        if (testCaseData) {
          ...
          setFormData({
            ...
            reviewer: testCaseData.reviewer || '',
            browsers: testCaseData.browsers || ['chrome'],
            environment: testCaseData.environment || ''
          });
          setSteps(testCaseData.steps || []);
        }
      } catch (error) {
        console.error('Error loading test case from localStorage:', error);
      }
    }
    setLoading(false);
  }, [projectId, testCaseId, router]);

  // Initialize display names when steps and elements are loaded
  useEffect(() => {
    if (steps.length > 0 && elements.length > 0) {
      const newActionDisplayNames: {[key: string]: string} = {};
      const newElementDisplayNames: {[key: string]: string} = {};
      
      steps.forEach((step: any) => {
        if (step.action) {
          newActionDisplayNames[`${step.id}-action`] = step.action.charAt(0).toUpperCase() + step.action.slice(1);
        }
        if (step.element) {
          // Find the element name from the elements list
          const element = elements.find(el => el.selector === step.element);
          if (element) {
            newElementDisplayNames[`${step.id}-element`] = element.name;
          }
        }
      });
      
      setActionDisplayNames(newActionDisplayNames);
      setElementDisplayNames(newElementDisplayNames);
    }
  }, [steps, elements]);
  ...
  const selectOption = (stepId: number, field: 'action' | 'element' | 'value' | 'description', value: string, displayName?: string) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedTestCases = existingTestCases.map((tc: any) => 
      tc.id.toString() === testCaseId ? updatedTestCase : tc
    );
    ...
    return (
      <div className="project-detail-layout">
        <div className="error-container">
          ...
          <p>The test case you're looking for doesn't exist.</p>
          <Link href={`/dashboard/project/${projectId}/test-cases`} className="btn-primary">Back to Test Cases</Link>
        </div>
      </div>
    );
  }
        ...
        <header className="project-header">
          <div className="header-content">
            <div className="header-left">
              <h1 className="project-title">Edit Test Case</h1>
            </div>
                        ...
                        <div className="step-fields">
                          ...
                          <div className="field-group">
                            ...
                            {errors[`step_${index}_action`] && (
                              <span className="error-message">{errors[`step_${index}_action`]}</span>
                            )}
                          </div>
                          <div className="field-group">
                            <label>
